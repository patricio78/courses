...
h = a3;
%[maxval,max_index] = max(h,[],2);
%repidx = repmat([1:num_labels], m, 1);
%repy = repmat(max_index, 1, num_labels);
%h = (repidx == repy);

repidx = repmat([1:num_labels], m, 1);
repy = repmat(y, 1, num_labels);
y = (repidx == repy);

cost = sum(-y .* log(h) - (1-y) .* log(1-h),2);
J = (1 / m) * sum(cost) + lambda / (2 * m) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
