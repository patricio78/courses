...
class robot:
    ...
    def move(self, motion,  tolerance = 0.001): 
        ...
        if (abs(turn) > tolerance):            
            ...
            x_new = cx + sin(self.orientation+turn)*radius
            y_new = cy - cos(self.orientation+turn)*radius
        else:
            x_new = self.x + distance*cos(self.orientation)
            y_new = self.y + distance*sin(self.orientation)
        
        orientation_new = (self.orientation+turn) % (2*pi)
