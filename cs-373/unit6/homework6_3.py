...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        Omega = Omega.expand(Omega.dimx+2,  Omega.dimy+2, [0, 1] + [i for i in range(4, Omega.dimx+2)],  [0, 1] + [i for i in range(4, Omega.dimy+2)])
        Xi = Xi.expand(Xi.dimx+2, Xi.dimy,  [0, 1] + [i for i in range(4, Xi.dimx+2)],  [0])
        ...
        B = Omega.take([0, 1])
        A = Omega.take([0, 1],  [i for i in range(2, Omega.dimy)])
        Omega_prime = Omega.take([i for i in range(2, Omega.dimx)],  [i for i in range(2, Omega.dimy)])
        
        C = Xi.take([0, 1],  [0])
        Xi_prime = Xi.take([i for i in range(2, Xi.dimx)],  [0])
