...
def plot_run(grid, plist):
    ...
    for r in range(len(grid)):
        for c in range(len(grid[0])):
            if grid[r][c]:
                circ = mtpl.Circle((c,r), radius=0.5)
                ax = mtpl.gca()
                ax.add_patch(circ)

    border = [[0,0], [0, cols], [rows, cols], [rows, 0], [0,0]]
    mtpl.plot([p[1]-.5 for p in border], [p[0]-.5 for p in border])
    mtpl.plot([(p[1]-.9)*1.2 for p in border], [(p[0]-.9)*1.2 for p in border])

    for plot in plist:
        points = plot[0]
        label = plot[1]
        mtpl.plot([p[1] for p in points], [p[0] for p in points], label=label)
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000, do_plot=False):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        if do_plot:
            my_estimated_route.append([estimate[0], estimate[1]])
            my_real_route.append([myrobot.x, myrobot.y])
    ...
    if do_plot:
        plot_run(grid, [[spath, "spath"],  [my_real_route, "REAL ROUTE"],  [my_estimated_route, "EST ROUTE"]])
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
     weight_data, weight_smooth, p_gain, d_gain, do_plot=False):

    path = plan(grid, init, goal)
    path.astar()
    path.smooth(weight_data, weight_smooth)
    return run(grid, goal, path.spath, [p_gain, d_gain], do_plot=do_plot)
...
print main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
           weight_data, weight_smooth, p_gain, d_gain,  False)
